...
class SakuraClient extends EventEmitter {
    ...
    processEvents() {
        if (this.options.command.initHandler) {
    ...
    loadCommands(commands) {
        for (let command of commands) {
            for (let name of command.names) {
                this.pluginCommands.set(name, command);
            } 
        }
    }
    ...
    loadEvents(events) {
        try {
            for (let event of events) {
                this.client[event.once ? "once" : "on"](event.event, executeEvent);
    
                function executeEvent(...data) {
                    return event.run(this.logger, ...data);
                }
            }
