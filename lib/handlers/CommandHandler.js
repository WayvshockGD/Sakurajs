...
module.exports = class CommandHandler {
    ...
    get responses() {
        return Object.assign(defaultResponses, this.client.options.responses);
    }
    ...
    setup(message) {
        ...
        if (!msg) return;
        if (this.options.defaultPrefix && !msg.content.startsWith(this.options.defaultPrefix)) return;
        if (this.options.ignoreBots && msg.author.bot) return;

        let args = msg.content.slice(this.options.defaultPrefix.length).trim().split(/ +/g);
        ...
        if (!command.enabled) {
            if (this.responses.commandDisabled) {
                return msg.post(this.responses.commandDisabled);
            }
            return;
        }
